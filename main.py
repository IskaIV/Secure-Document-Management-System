        ...
        try:
            ...
            with sqlite3.connect('database.db') as con:
                ...
                if (password == confirm_pass):
                    cur.execute("INSERT INTO Login (FSUID, Password, First, Last) VALUES (?,?,?,?)", (
...
def welcome():
    if request.method == "POST":
        ...
        try:
            ...
            cursor.execute(
                "SELECT * FROM Login WHERE FSUID = ? AND Password = ?", (fsuid, hashed_password))
    ...
    if request.method == "POST":
        ...
        try:
            ...
            with sqlite3.connect('database.db') as con:
                cur = con.cursor()
                cur.execute(
                    "INSERT INTO Posts (FSUID, Title,Description) VALUES (?,?,?)", (user[0], title, description))
        except:
            con.rollback()
            return render_template('Error.html')
        finally:
...
def post_detail(post_id):
    ...
    cur.execute(
        "SELECT Author, Content FROM Comments WHERE PostId = ? ORDER BY CommentId ASC", (post_id,))
...
def messages():
    ...
    cur.execute(
        "SELECT PrimaryUser, SecondaryUser FROM Replies WHERE PrimaryUser = ? OR SecondaryUser = ? ORDER BY Ind DESC", (user[0], user[0]))
    ...
    query = "SELECT FSUID, First FROM Login WHERE FSUID IN ({})".format(
        ', '.join('?' for _ in fsuids))
...
def sendmessages():
    if request.method == 'POST':
        ...
        try:
            ...
            cur.execute("SELECT PrimaryUser, Message FROM Replies WHERE (PrimaryUser = ? AND SecondaryUser = ?) OR (PrimaryUser = ? AND SecondaryUser = ?) ORDER BY Ind ASC",
                        (user[0], other_person, other_person, user[0]))
...
def sendMessages():
    if request.method == 'POST':
        ...
        try:
            ...
            cur = conn.cursor()
            cur.execute(
                "INSERT INTO Replies (PrimaryUser,SecondaryUser,Message) VALUES (?,?,?)", (user[0], other, message))
            conn.commit()
            cur.execute("SELECT PrimaryUser, Message FROM Replies WHERE (PrimaryUser = ? AND SecondaryUser = ?) OR (PrimaryUser = ? AND SecondaryUser = ?) ORDER BY Ind ASC",
                        (user[0], other, other, user[0]))
            authors_and_messages = cur.fetchall()
...
def comment():
    if request.method == "POST":
        ...
        try:
            ...
            cur = conn.cursor()
            cur.execute("INSERT INTO Comments (PostId,Content,Author) VALUES (?,?,?)",
                        (post_id, content, user[0]))
            conn.commit()
